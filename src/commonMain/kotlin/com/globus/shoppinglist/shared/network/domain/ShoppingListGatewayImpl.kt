...
class ShoppingListGatewayImpl(private val client: HttpClient) : ShoppingListGateway {
    ...
    override suspend fun login(phone: Long): BaseResponse<LoginResponse> {
        val request = client.get {
            url { path(loginPath) }
            parameter("phone", phone)
            //todo запилить к апи токен авторизации
    ...
    override suspend fun smsConfirm(
        code: String,
        userId: String
    ): BaseResponse<LoginConfirmResponse> {
    ...
    fun updateBearerToken(newToken: String) {
